...
joblib.dump(x_scaler, "scalerX2_G1.pkl")
joblib.dump(y_scaler, "scalerY2_G1.pkl")
...
model = keras.Sequential([
    ...
    keras.layers.Dense(16, kernel_regularizer=l2_reg),
    keras.layers.BatchNormalization(),
    keras.layers.ReLU(),
    #keras.layers.Dropout(0.2),


    keras.layers.Dense(32, kernel_regularizer=l2_reg),
    keras.layers.ReLU(),
...
checkpoint_cb = keras.callbacks.ModelCheckpoint(
    filepath="checkpoint_G1.best.weights.h5",
    monitor="val_loss",
    save_best_only=True,
...
# ======================================================
model.load_weights("checkpoint_G1.best.weights.h5")
...
plt.title("Loss G1 (MSE – spazio scalato)")
plt.xlabel("Epoche")
...
plt.title("MAE G1 (spazio scalato)")
plt.xlabel("Epoche")
...
plt.title("P reale vs P predetta – G1")
plt.grid(True)
plt.tight_layout()
plt.show()
